...
from modules.lobby import on_inactive_confirmed, lobby_init
...
def _add_main_handlers(client):
    ...
    async def on_command_error(ctx, error):
        if isinstance(error, commands.CommandNotFound):  # Unknown command
            if is_all_locked():
                await display.BOT_IS_LOCKED.send(ctx)
                return
            await display.INVALID_COMMAND.send(ctx)
            return
        if isinstance(error, commands.errors.CheckFailure):  # Unauthorized command
            cog_name = ctx.command.cog.qualified_name
            if cog_name == "admin":
                await display.NO_PERMISSION.send(ctx, ctx.command.name)
                return
            try:
                ...
                await display.WRONG_CHANNEL.send(ctx, ctx.command.name, channel_str)
            except KeyError:  # Should not happen
                await display.UNKNOWN_ERROR.send(ctx, "Channel key error")
            return
        ...
        if bl:
            ...
            await display.INVALID_STR.send(ctx, '"')
            return

        if isinstance(error.original, UnexpectedError):
            log.error(str(error))
            await display.UNKNOWN_ERROR.send(ctx, error.original.reason)
        else:
            # Print unhandled error
            log.error(str(error))
            await display.UNKNOWN_ERROR.send(ctx, type(error.original).__name__)
        raise error
        ...
        if payload.message_id == cfg.general["rules_msg_id"]:
            ...
            if str(payload.emoji) == "✅":
                ...
                if p.status is PlayerStatus.IS_NOT_REGISTERED:
                    ...
                    await display.REG_RULES(ContextWrapper.channel(cfg.channels["register"]), payload.member.mention)
